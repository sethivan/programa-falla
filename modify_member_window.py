...
class ModifyMemberWindow(tk.Toplevel):
	...
	def __init__(self, master = None):
		...
		self.family_id = 0
		self.family_ids = [] #variable per a controlar el camp idfamilia de la llista de noms del combo
		self.final_family_id = 0 #variable on guardem el valor final del idfamilia
		#self.modification = 0
		...
		self.label_name = ttk.Label(label_frame_data, text = "Nom:", style = "Etiqueta.TLabel")
		self.label_name.grid(row = 0, column = 0, padx = 7, pady = 2, sticky = "w")

		self.entry_name = ttk.Entry(label_frame_data, textvariable = self.name)
		self.entry_name.grid(row = 1, column = 0, padx = 7)
		self.entry_name.focus()

		self.label_surname = ttk.Label(label_frame_data, text = "Cognoms", style = "Etiqueta.TLabel")
		self.label_surname.grid(row = 0, column = 1, pady = 2, sticky = "w")

		self.entry_surname = ttk.Entry(label_frame_data, width = 30, textvariable = self.surname)
		...
		self.entry_birthdate = ttk.Entry(label_frame_data, width = 30, textvariable = self.birthdate)
		...
		self.entry_address = ttk.Entry(label_frame_data, width = 30, textvariable = self.address)
		...
		self.label_phone_number.grid(row = 6, column = 0, padx = 7, pady = 2, sticky = "w")

		self.entry_phone_number = ttk.Entry(label_frame_data, textvariable = self.phone_number)
		...
		self.entry_email = ttk.Entry(label_frame_data, width = 30, textvariable = self.email)
		self.entry_email.grid(row = 7, column = 1)

		# Frame Familiar del faller.
		self.label_family_option = ttk.Label(label_frame_family, text = "Familiar en la falla", style = "Etiqueta.TLabel")
		self.label_family_option.grid(row = 0, column = 0, padx = 5, pady = 2, sticky = "w")
		...
		self.radio_button_family_yes.grid(row = 1, column = 0, padx = 5, sticky = "w")
		self.radio_button_family_no.grid(row = 1, column = 0)

		self.label_family_name = ttk.Label(label_frame_family, text = "Cognoms i nom:", style = "Etiqueta.TLabel")
		self.label_family_name.grid(row = 2, column = 0, padx = 5, pady = 2, sticky = "w")

		self.combo_box_member_family = ttk.Combobox(label_frame_family, width = 30, postcommand = self.display_family)
		...
		# Botons.
		self.button_update = ttk.Button(self, text = "Actualitzar dades", style = "Boto.TButton", command = self.actualitzar)
	...
	def fill_in_fields(self, id):
		...
		for values in result:
			family_member = Member(values[0], values[1], values[2], values[3], values[4], values[5], values[6], values[7], values[8], values[11], family)
			family.members_list.append(family_member)
		family_members_list = []
		for family_member in family.members_list:
			if family_member.id != id:
				family_members_list = family_members_list + [(family_member.surname + ", " + family_member.name)]
	...
	def disable_family(self):
		...
		self.combo_box_member_family.delete(0, tk.END) # Borra tot el contingut de la llista.
		self.combo_box_member_family.set("") # Borra l'element que es queda a la vista.
		self.final_family_id = 0 # Borra l'identificador de familia en cas que s'haguera creat.
	...
	def display_family(self):
		...
		Utilitza l'atribut "self.family_ids" per a passar el identificador de familia a la funció "select_family".
		'''
		falla = Falla()
		surname = self.combo_box_member_family.get()
		falla.get_members("surname", surname)
		family_list = [] # Llista on anem a acumular els valors.
		self.family_ids = []
		...
		if valor=="yes":
			...
			try:
				...
				for values in result:
					...
					family.members_list.append(family_member)
				if family.id != self.final_family_id and self.final_family_id != 0:					
					# Si estava sol i entra en familia.
					if len(family.members_list) == 1:
						result = Family.get_family(self.final_family_id)
						family = Family(result[0], result[1], result[2])
						member.modify_member(self.id, member.name, member.surname, birthdate, member.gender, member.dni, member.address, member.phone_number, 1, member.email, family.id, category.id)
						family.delete_family(self.family_id)
						result = family.get_members(family.id)
						family.members_list = []
						for values in result:
							category_member = Category(values[15], values[16], values[17], values[18])
							family_member = Member(values[0], values[1], values[2], values[3], values[4], values[5], values[6], values[7], values[8], values[11], family, category_member)
							family.members_list.append(family_member)
					...
					# Si estava en familia i canvia de familia.
						...
						result = family.get_members(self.family_id)
						family.members_list = []
						for values in result:
							category_member = Category(values[15], values[16], values[17], values[18])
							family_member = Member(values[0], values[1], values[2], values[3], values[4], values[5], values[6], values[7], values[8], values[11], family, category_member)
				...
				elif len(family.members_list)>1 and self.final_family_id==0:
					Family.set_family(0, 0)
					result = Family.get_family(0)
					new_family = Family(result[0], result[1], result[2])
					member.modify_member(self.id, member.name, member.surname, birthdate, member.gender, member.dni, member.address, member.phone_number, 1, member.email, new_family.id, category.id) 
					# Actualitzem familia vella
					result = family.get_members(family.id)
					family.members_list = []
					for values in result:
