DROP PROCEDURE IF EXISTS sp.calculateMemberCategory;

DELIMITER $$
$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp`.`calculateMemberCategory`(IN vBirthdate DATETIME, OUT vIdCategory INT)
/*
A partir de la data de naixement del faller calcula la categoria a la que pertany.
@params vBirthdate: date
@params vIdCategory: int
*/
BEGIN
	DECLARE vFallaAge INT;
	CALL calculateMemberFallaAge(vBirthdate, vFallaAge);
    CASE 
        WHEN vFallaAge < 5 THEN
            SET vIdCategory = 5;
        WHEN vFallaAge >= 5 AND vFallaAge <= 9 THEN
            SET vIdCategory = 4;
        WHEN vFallaAge >= 10 AND vFallaAge <= 13 THEN
            SET vIdCategory = 3;
        WHEN vFallaAge >= 14 AND vFallaAge <= 17 THEN
            SET vIdCategory = 2;
        ELSE
            SET vIdCategory = 1;
    END CASE;
END
$$
DELIMITER ;